...
object AkkaClusterSupervisorSpec {
  val config = ConfigFactory.parseString("""
    ...
    spark {
      ...
      jobserver {
        job-result-cache-size = 100
        context-creation-timeout = 2 s
        yarn-context-creation-timeout = 2 s
...
// This test DOES NOT cover actually spawning a separate JVM,
class AkkaClusterSupervisorSpec extends TestKit(AkkaClusterSupervisorSpec.system) with ImplicitSender
  ...
  class StubbedAkkaClusterSupervisorActor(daoActor: ActorRef, dataManagerActor: ActorRef)
    ...
    override protected def startContext(
      ...
      contextInitInfos(actorName) = (contextConfig, isAdHoc, successFunc, failureFunc)
    }
  }
  ...
  var dataManagerActor: ActorRef = _
  ...
  before {
    dao = new InMemoryDAO
    val dataFileDao: DataFileDAO = new DataFileDAO(AkkaClusterSupervisorSpec.config)
    daoActor = system.actorOf(JobDAOActor.props(dao))
    dataManagerActor = system.actorOf(DataManagerActor.props(dataFileDao))
    supervisor = system.actorOf(Props(classOf[StubbedAkkaClusterSupervisorActor], this,
      daoActor, dataManagerActor))
  }
